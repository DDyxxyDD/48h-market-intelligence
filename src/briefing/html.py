...
def generate_html_briefing(
    selected: dict[str, list[Article]], sections: dict[str, Any], output_path: Path, live: bool = False
) -> Path:
    ...
    for key, settings in sections.items():
        ...
        for index, article in enumerate(selected.get(key, [])):
            analysis = analyze_article(article) if not live else None
            ...
            cards.append(f"""
              <article class="card">
                ...
                {f'<h3>Source summary</h3><p>{escape(article.summary or "No source-provided description available.")}</p><p class="disabled">AI analysis not enabled yet</p>' if live else f'''<h3>What happened</h3><p>{escape(analysis['what_happened'])}</p>
